...
from marker_coordinate import get_homogeneous_matrix, marker_coordinate
...
class MAKER_ESTIMATOR:
    def __init__(self, marker_length, marker_type, flag, input, num_marker):
        ...
        if flag == 'iphone13':
            ...
            self.camera_mat = np.array([2627.3684,0,2016,0,26273.684,1512,0,0,1], dtype=np.float32).reshape(3,3)
            self.distort_coefficient = np.array([0,0,0,0,0], dtype=np.float32)
        if flag == 'iphonex':
    ...
    def plot_img(self, camera_pose_post, save_name):
        ...
        #     camera_pose_post[-1] = 1
            ...
            self.count_list.append((self.count)*5)
    ...
    def draw_img(self, img, marker_idx, corner, s_txt, camera_pose_post, save_name):
        ...
        if marker_idx < 11:
            ...
            camera_pose_post = [round(x,2) for x in np.squeeze(tvec)]
            camera_pose_post.append(1.)
            ### new one homo
            # camera_pose_h = get_homogeneous_matrix(marker_idx) @ np.linalg.inv(pose_matrix)
...
if __name__ == '__main__':
    ...
    img_path = './test_images/2nd_test_images/'
    ftype = '.jpg'
